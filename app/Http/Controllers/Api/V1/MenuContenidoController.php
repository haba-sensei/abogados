...
class MenuContenidoController extends Controller
{
    ...
    {
        $zona_filter = DB::table('mapa_areas_practica')
            ->where('zona_id', $zona)
            ->where('detalle_cont_area_id', $det_id)
            ->get();
    ...
    {
        $imageables = DB::table('imageables')
            ->where('modelo_nivel', $nivel)
            ->where('menu_id', $menu)
            ->get();
    ...
    {
        $attacheable = DB::table('attacheable')
            ->where('modelo_nivel', $nivel)
            ->where('menu_id', $menu)
            ->get();
